...
1. Create shared folders for each folder that should be accessable in Windows plus one folder named `samba` (make sure this is not hidden in "My Network Drives")
2. Enable SSH (Control Panel > Terminal & SNMP) with High security level
3. Verify SMB is enabled (Control Panel > File Services > SMB) 
4. In Windows, open open any folder and browse to `\\ds713`
5. Right-click the folder `samba` and then *Map Network Drive* 
6. 
